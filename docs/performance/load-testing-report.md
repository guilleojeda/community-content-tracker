...
- Target: Local mock API (`load-tests/mock-server.js`) representing search and stats endpoints
- Dataset: Mock search responses simulate a 50,000-item catalog (`total: 50000`).
- Phases:
  1. Warm-up – 30s @ 50 rps
  2. Spike – 60s ramp to 1000 concurrent virtual users (maxVusers=1000)
  3. Stats burst – 30s sustained 200 req/s against `/api/stats`
...
3. Stats scenario completed successfully with consistent throughput and no HTTP-level failures.
