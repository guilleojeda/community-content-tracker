...
class KnowledgeBasesForAmazonBedrock:
    ...
    def create_bedrock_kb_execution_role(
        ...
        print("Waiting for IAM policy propagation (10 seconds)...")
        time.sleep(10)
    ...
    def create_s3_vectors_policy(
        ...
        # Wait for IAM policy propagation to prevent permission errors
        print("Waiting for IAM policy propagation (15 seconds)...")
        time.sleep(15)
        
        # Validate IAM permissions are working
        self._validate_iam_permissions(bedrock_kb_execution_role, vector_bucket_arn, index_arn)

    def _validate_iam_permissions(self, bedrock_kb_execution_role, vector_bucket_arn, index_arn):
        """
        Validate that IAM permissions have propagated by attempting to assume the role
        and check if it can access the required resources.
        """
        max_attempts = 5
        for attempt in range(max_attempts):
            try:
                # Try to assume the role to test if permissions are active
                ...
                assumed_role = sts_client.assume_role(
                    RoleArn=role_arn,
                    RoleSessionName=f'kb-validation-{int(time.time())}'
                )
                
                print(f"[WHITE HEAVY CHECK MARK] IAM role assumption successful on attempt {attempt + 1}")
                return True
                
            except ClientError as e:
                if attempt < max_attempts - 1:
                    print(f"IAM permissions not ready (attempt {attempt + 1}/{max_attempts}), waiting 5 seconds...")
                    time.sleep(5)
                else:
                    print(f"[WARNING SIGN] IAM validation failed after {max_attempts} attempts, proceeding anyway...")
                    return False
        return False
    ...
    @retry(
        ...
        retry_on_exception=lambda e: (
            ...
            (
                'is not authorized to perform' in str(e) or
    ...
    def create_knowledge_base(
        ...
        try:
            ...
            if 'is not authorized to perform' in error_message:
                print(f"[CROSS MARK] IAM Permission Error: {error_message}")
                print("[ELECTRIC LIGHT BULB] This is likely due to IAM policy propagation delay. Retrying...")
                raise e  # Let the retry decorator handle it
            elif error_code == 'ValidationException':
                print(f"[CROSS MARK] Validation Error: {error_message}")
                raise e  # Let the retry decorator handle it
            else:
                print(f"[CROSS MARK] Unexpected error creating Knowledge Base: {error_code} - {error_message}")
                raise e
        except Exception as e:
            print(f"[CROSS MARK] Unexpected error: {str(e)}")
            raise e
...
if __name__ == "__main__":
    ...
    if args.mode == "create":
        ...
        return_policy_kb_id, return_policy_ds_id = kb.create_or_retrieve_knowledge_base(
            data["return_policy_kb"]["name"], 
            data["return_policy_kb"]["description"]
        )
        print(f"Return Policy Knowledge Base ID: {return_policy_kb_id}")
        print(f"Return Policy Data Source ID: {return_policy_ds_id}")

        # Upload return policy files
        kb.upload_directory(
            f"{current_dir}/../{data['return_policy_kb']['files_path']}", 
            kb.get_data_bucket_name()
        )
        kb.synchronize_data(return_policy_kb_id, return_policy_ds_id)

        # Store Return Policy KB ID in SSM
        smm_client.put_parameter(
            Name="/app/customersupport/kb/return-policy-kb-id",
            Description=f"{data['return_policy_kb']['name']} kb id",
            Value=return_policy_kb_id,
            Type="String",
            Overwrite=True,
        )

        # Create Product Information Knowledge Base
        product_info_kb_id, product_info_ds_id = kb.create_or_retrieve_knowledge_base(
            data["product_info_kb"]["name"], 
            data["product_info_kb"]["description"]
        )
        print(f"Product Information Knowledge Base ID: {product_info_kb_id}")
        print(f"Product Information Data Source ID: {product_info_ds_id}")

        # Upload product info files
        kb.upload_directory(
            f"{current_dir}/../{data['product_info_kb']['files_path']}", 
            kb.get_data_bucket_name()
        )
        kb.synchronize_data(product_info_kb_id, product_info_ds_id)

        # Store Product Info KB ID in SSM
        smm_client.put_parameter(
            Name="/app/customersupport/kb/product-info-kb-id",
            Description=f"{data['product_info_kb']['name']} kb id",
            Value=product_info_kb_id,
            Type="String",
            Overwrite=True,
        )

    if args.mode == "delete":
        # Delete both knowledge bases
        kb.delete_kb(data["return_policy_kb"]["name"])
        kb.delete_kb(data["product_info_kb"]["name"])
        ...
        try:
            ...
            print("Return Policy KB parameter not found")
        ...
        try:
            ...
            print("Product Info KB parameter not found")
