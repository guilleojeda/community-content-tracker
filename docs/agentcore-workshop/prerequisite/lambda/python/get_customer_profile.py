...
def get_tier_emoji(tier: str) -> str:
    ...
    tier_emojis = {"Standard": "[THIRD PLACE MEDAL]", "Gold": "[FIRST PLACE MEDAL]", "Premium": "[GEM STONE]", "VIP": "[CROWN]"}
    return tier_emojis.get(tier, "[BUST IN SILHOUETTE]")
...
def get_customer_profile(
    ...
    try:
        ...
        if not customer_item:
            not_found_response = [
                "[CROSS MARK] Customer Profile Not Found",
                "=============================",
                f"[LEFT-POINTING MAGNIFYING GLASS] Search Method: {search_method}",
                f"[LEFT-POINTING MAGNIFYING GLASS] Search Value: {customer_id or email or phone}",
                "",
        ...
        profile_info = [
            "[BUST IN SILHOUETTE] Customer Profile Information",
            "===============================",
            f"Customer ID: {customer_id_value}",
            f"[BUST IN SILHOUETTE] Name: {first_name} {last_name}",
            f"{tier_emoji} Tier: {tier}",
            "",
            "[TELEPHONE RECEIVER] Contact Information:",
            f"   [E-MAIL SYMBOL] Email: {email_value}",
            f"   [MOBILE PHONE] Phone: {phone_value}",
            f"   [HOUSE BUILDING] Address: {formatted_address}",
            "",
            "[BAR CHART] Account Details:",
            f"   [CALENDAR] Registration Date: {registration_date}",
            f"   [BIRTHDAY CAKE] Date of Birth: {date_of_birth}",
            f"   Customer Since: {tenure_years} years, {tenure_months} months",
            "",
            "[BRIEFCASE] Purchase History:",
            f"   [SHOPPING TROLLEY] Total Purchases: {total_purchases}",
            f"   [MONEY BAG] Lifetime Value: ${lifetime_value:,.2f}",
            (
                f"   [DIRECT HIT] Average Order: ${(lifetime_value / total_purchases):,.2f}"
                if total_purchases > 0
                else "   [DIRECT HIT] Average Order: $0.00"
            ),
            "",
            "[HEADPHONE] Support Information:",
            f"   [TELEPHONE RECEIVER] Support Cases: {support_cases}",
            f"   [SPEECH BALLOON] Communication Preferences: {formatted_prefs}",
            "",
            "[MEMO] Account Notes:",
            f"   {notes}",
            "",
        ...
        if tier == "Premium" or tier == "VIP":
            ...
            profile_info.extend(
                [
                    "[FIRST PLACE MEDAL] Gold Benefits:",
        ...
        if support_cases > 3:
            recommendations.append(
                "[WARNING SIGN]  High support case count - consider proactive outreach"
            )

        if lifetime_value > 2000:
            recommendations.append("[GEM STONE] High-value customer - prioritize satisfaction")

        if tenure_years >= 2:
            recommendations.append("[PARTY POPPER] Loyal customer - consider loyalty rewards")
        ...
        if recommendations:
            profile_info.extend(
                [
                    "[ELECTRIC LIGHT BULB] Support Recommendations:",
        ...
        profile_info.extend(
            [
                "[HIGH VOLTAGE SIGN] Quick Actions Available:",
