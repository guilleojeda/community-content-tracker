...
def save_customer_support_secret(secret_value):
    ...
    try:
        ...
        print(f"[WHITE HEAVY CHECK MARK] Created secret")
    except secrets_client.exceptions.ResourceExistsException:
        secrets_client.update_secret(SecretId=secret_name, SecretString=secret_value)
        print(f"[WHITE HEAVY CHECK MARK] Updated existing secret")
    except Exception as e:
        print(f"[CROSS MARK] Error saving secret: {str(e)}")
        return False
    return True
...
def get_customer_support_secret():
    ...
    try:
        ...
        print(f"[CROSS MARK] Error getting secret: {str(e)}")
        return None
...
def delete_customer_support_secret():
    ...
    try:
        ...
        print(f"[WHITE HEAVY CHECK MARK] Deleted secret: {secret_name}")
        return True
    except Exception as e:
        print(f"[CROSS MARK] Error deleting secret: {str(e)}")
        return False
...
def create_agentcore_runtime_execution_role():
    ...
    policy_document = {
        ...
        "Statement": [
            ...
            },
            {
                ...
                "Resource": [
                    f"arn:aws:ssm:{region}:{account_id}:parameter/app/customersupport/agentcore/*"
                ],
            },
            {
    ...
    try:
        ...
        print(f"[WHITE HEAVY CHECK MARK] Created IAM role: {role_name}")
        ...
        try:
            ...
            print(f"[WHITE HEAVY CHECK MARK] Created policy: {policy_name}")
            policy_arn = policy_response["Policy"]["Arn"]

        # Attach policy to role
        try:
            iam.attach_role_policy(RoleName=role_name, PolicyArn=policy_arn)
            print(f"[WHITE HEAVY CHECK MARK] Attached policy to role")
        except Exception as e:
            if "already attached" in str(e).lower():
                print(f"INFO: Policy already attached to role")
            else:
                raise
        ...
        print(f"[CROSS MARK] Error creating IAM role: {str(e)}")
        return None
...
def delete_agentcore_runtime_execution_role():
    ...
    try:
        ...
        try:
            ...
            print(f"[WHITE HEAVY CHECK MARK] Detached policy from role")
        except Exception:
            pass
        ...
        try:
            ...
            print(f"[WHITE HEAVY CHECK MARK] Deleted role: {role_name}")
        except Exception:
            pass
        ...
        try:
            ...
            print(f"[WHITE HEAVY CHECK MARK] Deleted policy: {policy_name}")
        except Exception:
            pass
        ...
        print(f"[CROSS MARK] Error during cleanup: {str(e)}")
    ...
    while True:
        ...
        try:
            ...
            for memory in response.get('memories', []):
                ...
                response = control_client.list_memories(**params)
                print(f"[WHITE HEAVY CHECK MARK] Successfully deleted memory: {memory_id}")
            ...
            print(f"[WARNING SIGN]  Error getting memory details: {e}")
...
def gateway_target_cleanup():
    ...
    print(f"[WASTEBASKET]  Deleting all targets for gateway: {gateway_id}")
    ...
    for item in list_response["items"]:
        ...
        print(f"   [WHITE HEAVY CHECK MARK] Target {target_id} deleted")
    
    # Delete the gateway
    print(f"[WASTEBASKET]  Deleting gateway: {gateway_id}")
    gateway_client.delete_gateway(gatewayIdentifier=gateway_id)
    print(f"[WHITE HEAVY CHECK MARK] Gateway {gateway_id} deleted successfully")
...
def runtime_resource_cleanup():
    try:
        ...
        # print("  [WASTEBASKET]  Deleting AgentCore Runtime...")
        ...
        for runtime in runtimes['agentRuntimes']:        
            ...
            print(f"  [WHITE HEAVY CHECK MARK] Agent runtime deleted: {response['status']}")
        
        # Delete the ECR repository
        print("  [WASTEBASKET]  Deleting ECR repository...")
        ...
        for repo in repositories['repositories']:
            if 'bedrock-agentcore-customer_support_agent' in repo['repositoryName']:
                ...
                print(f"  [WHITE HEAVY CHECK MARK] ECR repository deleted: {repo['repositoryName']}")
      
    
    except Exception as e:
        print(f"  [WARNING SIGN]  Error during runtime cleanup: {e}")
...
def delete_observability_resources():
    ...
    try:
        print(f"  [WASTEBASKET]  Deleting log stream '{log_stream_name}'...")
        logs_client.delete_log_stream(
            logGroupName=log_group_name, logStreamName=log_stream_name
        )
        print(f"  [WHITE HEAVY CHECK MARK] Log stream '{log_stream_name}' deleted successfully")
    except Exception as e:
        if e.response["Error"]["Code"] == "ResourceNotFoundException":
            print(f"  INFO:  Log stream '{log_stream_name}' doesn't exist")
        else:
            print(f"  [WARNING SIGN]  Error deleting log stream: {e}")
    
    # Delete log group
    try:
        print(f"  [WASTEBASKET]  Deleting log group '{log_group_name}'...")
        logs_client.delete_log_group(logGroupName=log_group_name)
        print(f"  [WHITE HEAVY CHECK MARK] Log group '{log_group_name}' deleted successfully")
    except Exception as e:
        if e.response["Error"]["Code"] == "ResourceNotFoundException":
            print(f"  INFO:  Log group '{log_group_name}' doesn't exist")
        else:
            print(f"  [WARNING SIGN]  Error deleting log group: {e}")
    ...
    for file in files_to_delete:
        if os.path.exists(file):
            try:
                ...
                print(f"  [WHITE HEAVY CHECK MARK] Deleted {file}")
            except Exception as e:
                print(f"  [WARNING SIGN]  Error deleting {file}: {e}")
        else:
            missing_files.append(file)
    
    if deleted_files:
        print(f"\n[FILE FOLDER] Successfully deleted {len(deleted_files)} files")
    if missing_files:
        print(f"INFO:  {len(missing_files)} files were already missing: {', '.join(missing_files)}")
