...
class ChatManager:
    ...
    def display_chat_history(self):
        ...
        for message in messages_to_show:
            ...
            emoji = "[ADULT]‍[PERSONAL COMPUTER]" if message["role"] == "user" else "[ROBOT FACE]"
            ...
            with st.chat_message(message["role"]):
                if message["role"] == "assistant" and "elapsed" in message:
                    ...
                    create_safe_markdown_text(
                        f'<div class="{bubble_class}">{emoji} {clickable_content}<br><span style="font-size:0.9em;color:#888;">Response time: {message["elapsed"]:.2f} seconds</span></div>',
                        st,
                    )
                else:
    ...
    def process_user_message(self, prompt: str, actor_id: str, bearer_token: str):
        ...
        with st.chat_message("user"):
            create_safe_markdown_text(
                f'<span class="user-bubble">[ADULT]‍[PERSONAL COMPUTER] {prompt}</span>', st
            )
        ...
        with st.chat_message("assistant"):
            ...
            create_safe_markdown_text(
                '<span class="thinking-bubble">[ROBOT FACE] [THOUGHT BALLOON] Customer Support Assistant is thinking...</span>',
                message_placeholder,
            )
            ...
            for chunk in self.invoke_endpoint(
                ...
                if chunk.strip():
                    ...
                    create_safe_markdown_text(
                        f'<div class="assistant-bubble streaming typing-cursor">[ROBOT FACE] {clickable_streaming_text}</div>',
                        message_placeholder,
                    )
            ...
            create_safe_markdown_text(
                f'<div class="assistant-bubble">[ROBOT FACE] {clickable_streaming_text}<br><span style="font-size:0.9em;color:#888;">Response time: {elapsed:.2f} seconds</span></div>',
                message_placeholder,
            )
    ...
    def initialize_default_conversation(self, email, actor_id, bearer_token: str):
        ...
        if not st.session_state.messages:
            ...
            with st.chat_message("user"):
                create_safe_markdown_text(
                    f'<span class="user-bubble">[ADULT]‍[PERSONAL COMPUTER] {default_prompt}</span>', st
                )
                st.session_state["pending_assistant"] = True
            ...
            with st.chat_message("assistant"):
                ...
                create_safe_markdown_text(
                    f'<div class="assistant-bubble">[ROBOT FACE] {clickable_answer}<br><span style="font-size:0.9em;color:#888;">Response time: {elapsed:.2f} seconds</span></div>',
                    message_placeholder,
                )
