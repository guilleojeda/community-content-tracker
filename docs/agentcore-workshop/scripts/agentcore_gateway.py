...
def create_gateway(gateway_name: str, api_spec: List) -> dict:
    ...
    try:
        ...
        click.echo(f"[WHITE HEAVY CHECK MARK] Gateway created: {create_response['gatewayId']}")
        ...
        click.echo(f"[WHITE HEAVY CHECK MARK] Gateway target created: {create_target_response['targetId']}")
        ...
        click.echo("[WHITE HEAVY CHECK MARK] Gateway configuration saved to SSM parameters")

        return gateway

    except Exception as e:
        click.echo(f"[CROSS MARK] Error creating gateway: {str(e)}", err=True)
        sys.exit(1)
...
def delete_gateway(gateway_id: str) -> bool:
    ...
    try:
        click.echo(f"[WASTEBASKET]  Deleting all targets for gateway: {gateway_id}")
        ...
        for item in list_response["items"]:
            ...
            click.echo(f"   [WHITE HEAVY CHECK MARK] Target {target_id} deleted")

        # Delete the gateway
        click.echo(f"[WASTEBASKET]  Deleting gateway: {gateway_id}")
        gateway_client.delete_gateway(gatewayIdentifier=gateway_id)
        click.echo(f"[WHITE HEAVY CHECK MARK] Gateway {gateway_id} deleted successfully")

        return True

    except Exception as e:
        click.echo(f"[CROSS MARK] Error deleting gateway: {str(e)}", err=True)
        return False
...
def get_gateway_id_from_config() -> str:
    ...
    try:
        ...
        click.echo(f"[CROSS MARK] Error reading gateway ID from SSM: {str(e)}", err=True)
        return None
...
def create(name, api_spec_file):
    ...
    click.echo(f"[ROCKET] Creating AgentCore gateway: {name}")
    click.echo(f"[ROUND PUSHPIN] Region: {REGION}")

    # Validate API spec file exists
    if not os.path.exists(api_spec_file):
        click.echo(f"[CROSS MARK] API specification file not found: {api_spec_file}", err=True)
        sys.exit(1)

    try:
        api_spec = load_api_spec(api_spec_file)
        gateway = create_gateway(gateway_name=name, api_spec=api_spec)
        click.echo(f"[PARTY POPPER] Gateway created successfully with ID: {gateway['id']}")

    except Exception as e:
        click.echo(f"[CROSS MARK] Failed to create gateway: {str(e)}", err=True)
        sys.exit(1)
...
def delete(gateway_id, confirm):
    ...
    if not gateway_id:
        ...
        if not gateway_id:
            click.echo(
                "[CROSS MARK] No gateway ID provided and couldn't read from SSM parameters",
                err=True,
            )
            sys.exit(1)
        click.echo(f"[OPEN BOOK] Using gateway ID from SSM: {gateway_id}")

    # Confirmation prompt
    if not confirm:
        if not click.confirm(
            f"[WARNING SIGN]  Are you sure you want to delete gateway {gateway_id}? This action cannot be undone."
        ):
    ...
    if delete_gateway(gateway_id):
        click.echo("[WHITE HEAVY CHECK MARK] Gateway deleted successfully")
        ...
        click.echo("[BROOM] Removed gateway SSM parameters")

        # Clean up config file if it exists (backward compatibility)
        if os.path.exists("gateway.config"):
            os.remove("gateway.config")
            click.echo("[BROOM] Removed gateway.config file")

        click.echo("[PARTY POPPER] Gateway and configuration deleted successfully")
    else:
        click.echo("[CROSS MARK] Failed to delete gateway", err=True)
        sys.exit(1)
