...
# ----- 1. Create S3 bucket -----
echo "[BUCKET] Using S3 bucket: $FULL_BUCKET_NAME"
if [ "$REGION" = "us-east-1" ]; then
  aws s3api create-bucket \
    --bucket "$FULL_BUCKET_NAME" \
    2>/dev/null || echo "INFO: Bucket may already exist or be owned by you."
else
  aws s3api create-bucket \
    --bucket "$FULL_BUCKET_NAME" \
    --region "$REGION" \
...
# ----- 2. Zip Lambda code -----
echo "[PACKAGE] Zipping contents of $LAMBDA_SRC into $ZIP_FILE..."
cd "$LAMBDA_SRC"
zip -r "../../../$ZIP_FILE" . > /dev/null
cd - > /dev/null

# ----- 3. Upload to S3 -----
echo "[CLOUD] Uploading $ZIP_FILE to s3://$FULL_BUCKET_NAME/$S3_KEY..."
...
deploy_stack() {
  ...
  echo "[ROCKET] Deploying CloudFormation stack: $stack_name"
  ...
  if [ $exit_code -ne 0 ]; then
    if echo "$output" | grep -qi "No changes to deploy"; then
      echo "INFO: No updates for stack $stack_name, continuing..."
      return 0
    else
      echo "[CROSS MARK] Error deploying stack $stack_name:"
      echo "$output"
      return $exit_code
    fi
  else
    echo "[WHITE HEAVY CHECK MARK] Stack $stack_name deployed successfully."
    return 0
  fi
}

# ----- Run both stacks -----
echo "[WRENCH] Starting deployment of infrastructure stack..."
deploy_stack "$INFRA_STACK_NAME" "$INFRA_TEMPLATE_FILE" --parameter-overrides LambdaS3Bucket="$FULL_BUCKET_NAME" LambdaS3Key="$S3_KEY"
infra_exit_code=$?

echo "[WRENCH] Starting deployment of Cognito stack..."
deploy_stack "$COGNITO_STACK_NAME" "$COGNITO_TEMPLATE_FILE"
cognito_exit_code=$?

echo "[LEFT-POINTING MAGNIFYING GLASS] Fetching Knowledge Base and Data Source IDs from SSM..."
