...
export default function AdminUsersPage(): JSX.Element {
  ...
  const loadUsers = useCallback(
    async (offset = 0) => {
      ...
      setError(null);
      try {
        const response: AdminUserListResponse = await apiClient.listAdminUsers({
    ...
    [search, badgeFilter, pagination.limit]
  );
  ...
  const loadUserDetails = useCallback(
    async (userId: string) => {
      ...
      try {
        ...
        const flaggedContent = await apiClient.listFlaggedContent({ limit: 100, offset: 0 });
  ...
  const handleBadgeAction = async () => {
    ...
    try {
      if (badgeModal.mode === 'grant') {
        ...
        const result: AdminBulkBadgeResult = await apiClient.bulkBadges({
  ...
  const handleExportUsers = async () => {
    try {
      const { blob, filename } = await apiClient.exportUsersCsv();
  ...
  const toggleAwsEmployee = async (user: AdminUserDetail['user']) => {
    ...
    setActionMessage(null);
    try {
      await apiClient.setAwsEmployee(user.id, {
                ...
                <li key={item.id} className="rounded border border-red-100 bg-red-50 px-4 py-3 text-sm">
                  ...
                  <div className="mt-2 flex gap-2">
                    <button
                      ...
                      onClick={() => {
                        apiClient
                          .moderateContent(item.id, 'approve')
                          .then(() => loadUserDetails(selectedUser!.user.id))
                          .catch(() => setActionMessage('Failed to approve content.'));
                      }}
                    >
                      Approve
                    </button>
                    <button
                      ...
                      onClick={() => {
                        apiClient
                          .moderateContent(item.id, 'remove')
                          .then(() => loadUserDetails(selectedUser!.user.id))
                          .catch(() => setActionMessage('Failed to remove content.'));
                      }}
                    >
                      Remove
                    </button>
