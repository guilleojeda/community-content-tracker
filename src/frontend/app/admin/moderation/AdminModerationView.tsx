...
import type { FlaggedContentItem } from '@/api';
import { downloadBlob } from '@/utils/download';
const getSharedApiClient = async () => {
...
export default function AdminModerationPage(): JSX.Element {
  ...
  const loadFlaggedContent = async () => {
    ...
    try {
      ...
      const response = await client.listFlaggedContent({ limit: 100, offset: 0 });
  ...
  const handleModerationAction = async (
    ...
    try {
      const client = await getSharedApiClient();
      if (action === 'delete') {
        await client.adminDeleteContent(item.id, 'Removed by admin moderation');
      } else {
        await client.moderateContent(item.id, action, 'Admin moderation panel');
      }
